...
if (selectedFilm) {
  ...
  const date = document.querySelector("#premiera");

  const formattedDate = dayjs(selectedFilm.premiera).format("D. M. YYYY.");
  const differents = Math.abs(
    dayjs(selectedFilm.premiera).diff(dayjs(), "days")
  );
  let days = "dní.";
  if (differents === 1) {
    days = "den.";
  ...
  date.textContent =
    "Premiera " + formattedDate + ", což je za " + differents + " " + days;
}

const stars = document.querySelectorAll(".fa-star");

const chooseStars = (number) => {
